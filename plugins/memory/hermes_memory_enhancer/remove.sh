...
# other provider data, Python packages, Docker images, or server data.
...
Options:
 ...
 --purge-app-db Delete only this program's app-owned SQLite DB if it is under
               /memory_enhancer/ (never deletes system SQLite or other DBs)
...
What this removes:
 ...
 - optionally this program's own SQLite DB under /memory_enhancer/

What this never removes:
 - SQLite databases (without --purge-app-db)
 - MEMORY.md / USER.md
...
✅ Memory Enhancer provider disabled for profile at $HERMES_HOME_DIR

  memory.provider removed from config.yaml
  $( [[ "$REMOVE_ENV" -eq 1 ]] && echo "MEMORY_ENHANCER_* lines removed from .env" || echo ".env values preserved (not removed)" )
  $( [[ "$PURGE_APP_DB" -eq 1 ]] && echo "App-owned SQLite DB purged: $DB_PATH" || echo "SQLite DB preserved (use --purge-app-db to delete)" )

Restart Hermes CLI/gateway after making these changes.
EOF
