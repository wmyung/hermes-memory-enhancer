...
class FakeMemoryEnhancerClient:
...
class TestMemoryEnhancerSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert HermesMemoryEnhancerProvider._normalize_summary_uri("memory://user/hermes/.overview.md") == "memory://user/hermes"
        assert HermesMemoryEnhancerProvider._normalize_summary_uri("memory://resources/.abstract.md") == "memory://resources"
        assert HermesMemoryEnhancerProvider._normalize_summary_uri("memory://") == "memory://"
        assert HermesMemoryEnhancerProvider._normalize_summary_uri("memory://user/hermes/memories/profile.md") == "memory://user/hermes/memories/profile.md"


class TestMemoryEnhancerRead:
    def test_overview_read_normalizes_uri_and_unwraps_result(self):
        provider = HermesMemoryEnhancerProvider()
        provider._client = FakeMemoryEnhancerClient(
            {
                (
                    "/api/v1/content/overview",
                    (("uri", "memory://user/hermes"),),
                ): {"result": {"content": "overview text"}},
            }
        )

        result = json.loads(provider._tool_read({"uri": "memory://user/hermes/.overview.md", "level": "overview"}))

        assert result["uri"] == "memory://user/hermes/.overview.md"
        assert result["resolved_uri"] == "memory://user/hermes"
        ...
        provider._client = FakeMemoryEnhancerClient(
            {
                (
                    "/api/v1/content/read",
                    (("uri", "memory://user/hermes/memories/profile.md"),),
                ): {"result": "full text"},
            }
        )

        result = json.loads(provider._tool_read({"uri": "memory://user/hermes/memories/profile.md", "level": "full"}))

        assert result["uri"] == "memory://user/hermes/memories/profile.md"
        assert result["resolved_uri"] == "memory://user/hermes/memories/profile.md"
    ...
    def test_overview_file_uri_routes_straight_to_content_read_via_stat_probe(self):
        ...
        provider = HermesMemoryEnhancerProvider()
        file_uri = "memory://user/hermes/memories/entities/mem_abc.md"
        provider._client = FakeMemoryEnhancerClient(
            {
                (
                    "/api/v1/fs/stat",
    ...
    def test_overview_dir_uri_skips_stat_when_pseudo_summary(self):
        ...
        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "memory://user/hermes"}),
        ]
    ...
    def test_overview_directory_uri_uses_stat_probe_then_overview(self):
        ...
        provider = HermesMemoryEnhancerProvider()
        dir_uri = "memory://user/hermes/memories"
        provider._client = FakeMemoryEnhancerClient(
            {
                (
                    "/api/v1/fs/stat",
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "memory://user/hermes/.overview.md", "level": "overview"})
...
class TestMemoryEnhancerBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeMemoryEnhancerClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "memory://user/hermes/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "memory://user/hermes/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "memory://user/hermes"}))

        assert result["path"] == "memory://user/hermes"
        assert result["entries"] == [
            {"name": "memories", "uri": "memory://user/hermes/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "memory://user/hermes/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "memory://user/hermes"},
